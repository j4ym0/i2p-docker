...
RUN apt-get update && \
	apt-get install --no-install-recommends -y ca-certificates software-properties-common curl gnupg procps openjdk-21-jre-headless && \
	apt-add-repository -y ppa:i2p-maintainers/i2p && \
	apt-get update && \
	apt-get install -y i2p=2.9.0* && \
	mkdir /i2p && \
	groupadd -g 99 i2p && \
	useradd -r -u 99 -g i2p i2p --home /i2p && \
	chown -R i2p:i2p /i2p && \
